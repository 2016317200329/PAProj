...
from Config import config_GT2_sft_bidfee
import loss
import plot
...
reload(config_GT2_sft_bidfee)  # 必须reload！！
...
opt = config_GT2_sft_bidfee.DefaultConfig()
...
def get_model_save_path(flag,seed):
    ...
    model_params_MLP = opt.net_root_path + "NN_params_infer_sft_bidfee.pth"
...
def get_params(mlp,opt):
    ...
    params = [{'params': other_params},
                {'params': mlp.block_alpha.parameters(), 'lr': 5e-3},
...
def trainer(train_loader, val_loader, test_loader, mlp, opt, device):
    ...
    for epoch in range(EPOCH_NUM):
        ...
        # 这样最后training结束时的loss一定是loss最小的那个model
