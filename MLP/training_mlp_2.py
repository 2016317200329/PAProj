...
def get_params(mlp,config,opt):
    ...
    params = [{'params': base_params},  # 如果对某个参数不指定学习率，就使用最外层的默认学习率
            ...
            {'params': mlp.z_sigma.parameters(), 'lr': config['lr_for_sigma']}]

    return params

class MLP_1_1(nn.Module):
    # code->generate->override methods
    # ["relu","elu","softplus","leakyrelu"]
    def choose_ac(self,ac_str):
        if ac_str == "relu":
            return nn.ReLU()
        if ac_str == "elu":
            return nn.ELU()
    ...
    def __init__(self, n_gaussians,kernel_size, stride, ac_str1,ac_str2,ac_str3,ac_str_sigma) -> None:
        super().__init__()
        ...
        self.ac_func0 = nn.PReLU()
        self.ac_func1 = self.choose_ac(ac_str1)
        self.ac_func2 = self.choose_ac(ac_str2)
        self.ac_func3 = self.choose_ac(ac_str3)
        ...
        self.ac_str_sigma = ac_str_sigma
    ...
    def forward(self, x):
        ...
        x1 = self.ac_func0(self.conv1(x))                   # [B, 1, 1, ln_in]
        x2 = self.ac_func0(self.conv2(x))                   # [B, 1, 1, ln_in]
        x3 = self.ac_func0(self.conv3(x))                   # [B, 1, 1, ln_in]
        ...
        x1_3 = self.ac_func1(x1_2)
        x2_3 = self.ac_func2(x2_2)
        x3_3 = self.ac_func3(x3_2)

        pi = self.z_pi(x1_3)
        mu = self.z_mu(x2_3)

        sigma = self.z_sigma(x3_3)

        if self.ac_str_sigma=="exp":
            sigma = torch.exp(sigma)
        elif self.ac_str_sigma=="elu":
            sigma = F.elu(sigma)+1
        elif self.ac_str_sigma=="relu":
            sigma = F.relu(sigma)
...
class Objective:
    ...
    def __call__(self, trial):
        ...
        config = {
            'lr_for_sigma': trial.suggest_categorical('lr_for_sigma',[1e-3,5e-3,1e-2]),
            'ac_func3': trial.suggest_categorical('ac_func3',["rreLU","preLU","leakyrelu"]),
            'ac_str_sigma': trial.suggest_categorical('ac_str_sigma',["relu","exp","elu"])
        }

        train_loader, val_loader, test_loader = get_data_loader(self.dataset, self.shuffled_indices, self.opt)

        model = MLP_1_1(self.opt.N_gaussians, 12, 3,"preLU","leakyrelu",config['ac_func3'],config['ac_str_sigma']).to(device)  # put your model and data on the same computation device.
...
study_name = 'ac_func-lr_sigma'  # Unique identifier of the study.
...
# objective会被调用这么多次
study.optimize(Objective(dataset,opt), n_trials=1000,show_progress_bar=True)
