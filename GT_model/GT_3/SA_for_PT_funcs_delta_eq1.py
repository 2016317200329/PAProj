...
def f_2(x,alpha):
    ...
    return np.exp(-alpha*x)  # warning!!! overflow!!
...
def f_Equi(t,v,d,b,alpha):
    ...
    tmp = v - d * (t - 1) - b
    # root =  f(tmp,alpha) / (f(tmp,alpha) - f(-b,alpha))
    root = 1- (f_2(b,alpha)-1) / (f_2(tmp,alpha)-1)
    # assert root >= 0, f"Before clip, U[t] < 0 , tmp = {tmp}, U[t] = {root}, t,v,d,b,alpha = {t,v,d,b,alpha}"
    # assert root <= 1, f"Before clip, U[t] > 1 , tmp = {tmp}, U[t] = {root}, t,v,d,b,alpha = {t,v,d,b,alpha}"
...
def U_GT3(t,v, d, b,alpha = -0.0135, eps = 1e-10):
    return np.clip(f_Equi(t, v, d, b, alpha),eps,1-eps)  # 注意设置LEN之内，eps < u < 1-eps

def get_U_GT3(LEN,v, d, b, alpha = -0.0135,eps = 1e-10):
    """
    计算LEN之内的U. 注意：eps < U[t] < 1-eps, when 1 <= t <= LEN, and U[LEN+1] = 0
    Args:
        LEN: 可计算的最大的auction duration,它一定不會超過T
        v:
        d:
        b:
        alpha:
